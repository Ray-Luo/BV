...
def vis_image(img, ax=None):
    ...
    """
    if ax is None:
        fig = plot.figure(figsize = (img.shape[2]/128, img.shape[1]/128))
        # fig = plot.figure()
...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    if ax is None:
        fig = plot.figure(figsize = (img.shape[2]/160, img.shape[1]/160))
        # fig = plot.figure()
        ax = fig.add_subplot(1, 1, 1)
        ax.set_axis_off()
    # CHW -> HWC
    ...
    for i, bb in enumerate(bbox):
        ...
        ax.add_patch(plot.Rectangle(
            xy, width, height, fill=False, edgecolor='red', linewidth=4)) #2
    ...
    fig.savefig('/home/kdd/Documents/simple-faster-rcnn-pytorch/misc/output.png')
    return ax
